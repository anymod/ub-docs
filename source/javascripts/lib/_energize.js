...
(function () {
  ...
  touchmove = function (e) {
    ...
    this.threshold = isThresholdReached(this.startXY, [
      e.touches[0].clientX,
      e.touches[0].clientY,
    ]);
  };
  ...
  touchend = function (e) {
    ...
    if (
      this.threshold ||
      isThresholdReached(this.startXY, [
  ...
  click = function (e) {
    ...
    var time = Date.now(),
      ...
      isLink = nodeName === "A",
      standAlone =
        window.navigator.standalone && isLink && e.target.getAttribute("href");
    ...
    if (
      (!e.simulated &&
        (timeDiff < 500 ||
          (timeDiff < 1500 && xyDiff[0] < 50 && xyDiff[1] < 50))) ||
      standAlone
    ) {
      e.preventDefault();
      e.stopPropagation();
